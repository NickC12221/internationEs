...
export default function ImagesPage() {
  ...
  const fetchImages = async () => {
    try {
      ...
      if (data.success && data.data.profile) {
        ...
        setProfileId(data.data.profile.id)
      }
    } finally {
      setLoading(false)
    }
  ...
  const uploadImage = async (file: File) => {
    ...
    const saveRes = await fetch('/api/profile-images', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ key: data.key, url: data.publicUrl, order: images.length, isMain: images.length === 0 }),
    })
  ...
  const onDrop = useCallback(async (acceptedFiles: File[]) => {
    if (images.length >= 15) { alert('Maximum 15 photos allowed'); return }
    setUploading(true)
    setUploadProgress(0)
    for (let i = 0; i < acceptedFiles.length; i++) {
      if (images.length + i >= 15) break
  ...
  const deleteImage = async (imageId: string) => {
    ...
    await fetch(`/api/profile-images/${imageId}`, { method: 'DELETE' })
    ...
    <div className="min-h-screen bg-stone-950">
      ...
      <div className="mx-auto max-w-3xl px-4 py-10 sm:px-6">
        <div className="mb-6 flex items-center gap-3">
          ...
          <span className="ml-auto text-sm text-stone-500">{images.length} / 15 photos</span>
        </div>

        {/* Dropzone */}
        <div {...getRootProps()} className={`mb-8 cursor-pointer rounded-2xl border-2 border-dashed p-10 text-center transition-colors ${isDragActive ? 'border-amber-600 bg-amber-950/20' : 'border-stone-700 bg-stone-900 hover:border-amber-800 hover:bg-stone-800'} ${(uploading || images.length >= 15) ? 'opacity-60 cursor-not-allowed' : ''}`}>
          <input {...getInputProps()} />
          {uploading ? (
            <div className="flex flex-col items-center gap-3">
              ...
              <div className="w-48 h-1.5 bg-stone-700 rounded-full overflow-hidden">
                <div className="h-full bg-amber-600 rounded-full transition-all" style={{ width: `${uploadProgress}%` }} />
              </div>
            </div>
          ) : images.length >= 15 ? (
            <p className="text-sm text-stone-500">Maximum 15 photos reached</p>
          ) : (
            <div className="flex flex-col items-center gap-3">
              <Upload className="h-8 w-8 text-stone-500" />
              <div>
                <p className="text-sm font-medium text-stone-300">{isDragActive ? 'Drop photos here' : 'Drag & drop photos, or click to browse'}</p>
                <p className="mt-1 text-xs text-stone-600">JPEG, PNG, WebP · Up to 15 photos · Max 10MB each</p>
              </div>
            </div>
          )}
        </div>
        ...
        {loading ? (
          <div className="grid grid-cols-3 gap-3 sm:grid-cols-4">
            {[...Array(6)].map((_, i) => <div key={i} className="aspect-square rounded-xl bg-stone-900 animate-pulse" />)}
          </div>
        ) : images.length === 0 ? (
        ...
        <p className="mt-6 text-xs text-stone-600 text-center">The photo marked "Main" will appear as your profile picture.</p>
      </div>
    </div>
  )
}
