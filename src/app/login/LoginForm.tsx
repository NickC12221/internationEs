    ...
    <div className="flex min-h-screen items-center justify-center bg-stone-950 px-4">
      <div className="w-full max-w-md">
        ...
        <form onSubmit={handleSubmit} className="rounded-2xl border border-stone-800 bg-stone-900 p-8">
          {error && (
            <div className="mb-4 rounded-lg bg-red-950 border border-red-900 px-4 py-3 text-sm text-red-400">{error}</div>
          )}
          ...
          <p className="mt-4 text-center text-sm text-stone-500">
            ...
            <Link href="/signup" className="text-amber-500 hover:text-amber-400">Create one</Link>
          </p>
        </form>
        <p className="mt-6 text-center text-xs text-stone-600">
          <Link href="/" className="hover:text-stone-400">← Back to directory</Link>
        </p>
      </div>
    </div>
  )
