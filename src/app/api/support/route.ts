  ...
  try {
    ...
    if (existing) {
      ...
      const convo = await prisma.conversation.create({
        data: {
          subject: 'support',
          members: { create: [{ userId: session.id }, { userId: admin.id }] }
        }
      })
    ...
    await prisma.message.create({
      data: { conversationId, senderId: session.id, content: message || 'Hello, I need support.' }
    })
